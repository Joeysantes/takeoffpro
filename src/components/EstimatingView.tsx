...
function computeTotal(m: {
  type: string; value: number; height?: number; priceMode?: PriceMode; unitCost: number;
}): number {
...
export default function EstimatingView() {
  const { project, updateMeasurement } = useTakeoffStore();
  ...
  const filtered =
    pageFilter === 'all'
      ? allMeasurements
      : allMeasurements.filter((m) => m.pageIndex === pageFilter);
    ...
    <div className="flex-1 flex flex-col overflow-hidden bg-white">
      ...
      <div className="h-12 border-b border-zinc-200 flex items-center px-4 gap-3 shrink-0 bg-zinc-50">
        ...
        <div className="ml-auto flex gap-2">
          ...
          <button
            className="px-3 py-1.5 text-sm rounded-md border border-zinc-200 text-zinc-400 cursor-not-allowed"
            title="Company template import — coming soon"
            disabled
          >
            Template
          </button>
          ...
          <table className="w-full text-xs border-collapse">
            ...
            <tbody>
              {Object.entries(grouped).map(([trade, items]) => {
                ...
                return (
                  <>
                    <tr key={`${trade}-header`} className="bg-zinc-50">
                      <td
                        colSpan={9}
                        className="px-3 py-1.5 font-semibold text-zinc-600 text-xs uppercase tracking-wide border-t border-zinc-200"
                      >
                        {trade}
                      </td>
                    </tr>
                      ...
                      <tr key={m.id} className="border-b border-zinc-100 hover:bg-zinc-50">
                        ...
                        <td className="px-3 py-1.5 text-right">
                          <input
                            ...
                            step={0.1}
                            onChange={(e) =>
                              updateMeasurement(m.id, { height: parseFloat(e.target.value) || undefined })
                            }
                          />
                        </td>
                        <td className="px-3 py-1.5 text-right">
                          <input
                            ...
                            step={0.01}
                            onChange={(e) =>
                              updateMeasurement(m.id, { unitCost: parseFloat(e.target.value) || 0 })
                            }
                          />
                        </td>
                        <td className="px-3 py-1.5">
                          <input
                            className="border border-zinc-200 rounded px-1.5 py-0.5 text-xs font-mono bg-white w-48"
                            value={m.formula ?? ''}
                            placeholder="formula"
                            onChange={(e) => updateMeasurement(m.id, { formula: e.target.value })}
                          />
                        </td>
                        <td className="px-3 py-1.5 text-right font-semibold text-zinc-800">
                          ${computeTotal(m).toFixed(2)}
                        </td>
                      </tr>
                    ))}
                    <tr key={`${trade}-sub`} className="bg-zinc-50 border-b border-zinc-200">
                      <td colSpan={8} className="px-3 py-1.5 text-right text-zinc-500 font-medium">
                        {trade} subtotal
                      </td>
              ...
              <tr className="bg-blue-50 border-t-2 border-blue-300 sticky bottom-0">
                <td colSpan={8} className="px-3 py-2 text-right font-bold text-zinc-800">
                  Grand Total
                </td>
